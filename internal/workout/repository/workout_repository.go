...
func (r *WorkoutRepository) Create(ctx context.Context, userID uuid.UUID, templateID *uuid.UUID, scheduledAt *time.Time) (*workoutdomain.Workout, error) {
	query := `
		INSERT INTO workouts (template_id, user_id, scheduled_at)
		VALUES ($1, $2, $3)
		RETURNING id, template_id, user_id, scheduled_at, started_at, finished_at, created_at
	`
	var w workoutdomain.Workout
	err := r.pool.QueryRow(ctx, query, templateID, userID, scheduledAt).Scan(
		&w.ID, &w.TemplateID, &w.UserID, &w.ScheduledAt, &w.StartedAt, &w.FinishedAt, &w.CreatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (r *WorkoutRepository) GetByID(ctx context.Context, id uuid.UUID) (*workoutdomain.Workout, error) {
	query := `
		SELECT id, template_id, user_id, scheduled_at, started_at, finished_at, created_at
		FROM workouts
		WHERE id = $1
	`
	var w workoutdomain.Workout
...
func (r *WorkoutRepository) ListByUserID(ctx context.Context, userID uuid.UUID, limit, offset int) ([]*workoutdomain.Workout, error) {
	...
	for rows.Next() {
		var w workoutdomain.Workout
		if err := rows.Scan(&w.ID, &w.TemplateID, &w.UserID, &w.ScheduledAt, &w.StartedAt, &w.FinishedAt, &w.CreatedAt); err != nil {
			return nil, err
		}
		list = append(list, &w)
	}
