...
- [ ] Ограничить `Free User` до 3 тренировок в неделю (считать обычные + групповые).
- [ ] Ограничить `Free Coach` до 3 созданных групповых тренировок в неделю.
...
- [ ] 4-я тренировка за неделю для `Free User` -> paywall.
- [ ] 4-я созданная групповая за неделю для `Free Coach` -> paywall.
- [ ] Все блокировки показывают понятный CTA на оплату.

## 5.4 Навигация в тарифы

- [ ] Кнопка `Тарифы` на `home`.
- [ ] Переход в тарифы из профиля пользователя (под статусом подписки).
- [ ] Переход в тарифы из профиля тренера.
- [ ] Переходы используют устойчивую навигацию без navigator assertion.
